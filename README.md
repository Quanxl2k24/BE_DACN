...
- **`lookupKey` băm SHA-256 của refresh token**: Refresh token gốc được hash bằng `argon2` trước khi lưu (không thể đảo ngược để so khớp nhanh), nhưng để tra cứu theo token khi gọi refresh/logout mà không phải duyệt toàn bộ bảng, hệ thống lưu thêm `lookupKey = sha256(refreshToken)` làm khoá tra cứu — argon2 dùng để xác minh, SHA-256 dùng để định vị bản ghi.
